  ...
  <div>
      ...
      <div class="action-confirm" v-if="deleteID">
        <div class="card">
          <div class="card-header">
            Tem certeza que quer deletar a recompensa?
          </div>
          <div class="card-body">
            Essa ação não pode ser desfeita!
          </div>
          <div class="card-footer">
            <button class="btn btn-danger mr-2" v-on:click="deleteReward(deleteID)">Deletar</button>
    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            Recompensas <span class="float-right">Pontos Disponíveis: <b>{{ user.current_score }}</b></span>
          </div>
          <div class="card-body">
            <div class="row mb-3">
              <div class="col-3">
                ...
                <tbody>
                  ...
                  <template v-else>
                    <tr v-for="data in tableData.data" :key="data.id">
                      ...
                      <td v-tooltip="data.formatted_price">{{ data.formatted_price }}</td>
                      <td>
                        <div class="btn-group btn-group-sm">
                          <button :class="{'disabled': data.price > user.current_score}" class="btn btn-sm btn-success" type="button" v-on:click="buyReward(data)"><fa icon="shopping-cart"/></button>
                          <button class="btn btn-sm btn-primary" type="button" v-on:click="goToReward(data.id)"><fa icon="pen-square"/></button>
            ...
            <div class="table-footer row align-items-center" v-if="tableData.data && tableData.data.length > 0">
              ...
              <div class="col-6 text-center">
                <div class="btn-group">
                  <button class="btn btn-outline-primary" :disabled="tableData.current_page === 1" v-on:click="getRewards(1)">
                    <fa icon="caret-left"/>
                    <fa icon="caret-left"/>
                  </button>
                  <button class="btn btn-outline-primary" :disabled="tableData.current_page === 1" v-on:click="getRewards(tableData.current_page-1)">
                    <fa icon="caret-left"/>
                  </button>
                  <button v-for="button in tableButtons" :key="button" class="btn"
                          ...
                          v-on:click="getRewards(button)">
                    {{ button }}
                  </button>
                  <button class="btn btn-outline-primary" :disabled="tableData.current_page === tableData.last_page" v-on:click="getRewards(tableData.current_page+1)">
                    <fa icon="caret-right"/>
                  </button>
                  <button class="btn btn-outline-primary" :disabled="tableData.current_page === tableData.last_page" v-on:click="getRewards(tableData.last_page)">
                    <fa icon="caret-right"/>
                    <fa icon="caret-right"/>
                  </button>
                </div>
    ...
    <button class="btn btn-circle btn-primary btn-add btn-lg" v-on:click="goToReward(null)">
      <fa icon="plus"></fa>
    </button>
  </div>
</template>
    ...
    export default {
        ...
        methods: {
            buyReward: function (reward) {
                if (reward.price <= this.user.current_score) {
                    this.$http({
                        url: '/buy_reward',
                        method: 'put',
                        params: {
            ...
            goToReward: function (id) {
                this.$store.state.loading = true;
                this.$router.push({ name: 'reward', params: { id: id }});
            }
        },
        mounted: function() {
